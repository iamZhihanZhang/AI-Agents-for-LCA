...
_GOOGLE_PATTERNS = (
    ...
    re.compile(
        r"Annual energy use estimate(?:[0-9]+ kWh(?:/y)?)\s*(?P<energy_demand>[0-9]+) kWh(?:/y)?"
    ),
...
_TRANSPORT_PERCENT_PATTERN = re.compile(
    r"(Distribution|Transpor.*)\s*([0-9]*\.*[0-9]*)\s*%"
)
...
def parse(body: BinaryIO, pdf_filename: str) -> Iterator[data.DeviceCarbonFootprint]:
    ...
    if "name" in extracted:
        ...
        result["name"] = (
            first_page_text.replace("Product environmental report", "")
            .strip()
            .removeprefix("Google ")
        )
    ...
    for block, page in pdf.search_text(body, "Customer use"):
        ...
        use_text = page.get_textbox(
            (block.x0, block.y0, block.x1, block.y1 * 2.1 - block.y0)
        )
