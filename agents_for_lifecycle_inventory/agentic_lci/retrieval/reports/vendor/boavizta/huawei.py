...
_HUAWEI_DESK_PATTERNS = (
    ...
    re.compile(
        r" Total greenhouse gas emissions.?: (?P<footprint>[0-9]+(?:.[0-9]+)?)\s*kg ?CO2 ?e"
    ),
...
def parse(body: BinaryIO, pdf_filename: str) -> Iterator[data.DeviceCarbonFootprint]:
    ...
    if "name" not in extracted:
        for rect, page in pdf.search_text(body, "Product:"):
            line = page.get_textbox(
                (rect.x0, rect.y0 - 2, rect.x1 * 5 - rect.x0 * 4, rect.y1 + 2)
            )
    ...
    if "type" not in extracted:
        for rect, page in pdf.search_text(body, "Product type:"):
            line = page.get_textbox(
                (rect.x0, rect.y0, rect.x1 * 5 - rect.x0 * 4, rect.y1)
            )
    ...
    if "name" in extracted:
        result["name"] = (
            extracted["name"].strip().removeprefix("Huawei ").removeprefix("HUAWEI ")
        )
