...
_MS_PATTERNS = (
    ...
    re.compile(r"(?P<lifetime>[0-9]*) years of product use"),
    re.compile(r"(?P<lifetime>[0-9]*.[0-9]*) years of product use"),
    re.compile(r"assuming a (?P<lifetime>[a-z]+) year total use period"),
    re.compile(r"assuming a (?P<lifetime>[0-9]*)-year total use period"),
    re.compile(r"assuming a (?P<lifetime>[0-9]*.[0-9]*)-year total use period"),
    re.compile(
        r"Global warming potential\s*(?P<footprint>[0-9]*.[0-9]*)\s*kg\s*CO2.equivalent"
    ),
    re.compile(
        r"Greenhouse gas emissions\s*(?P<footprint>[0-9]*.[0-9]*)\s*kg\s*CO2.equivalent"
    ),
    re.compile(
        r"full life cycle carbon footprint2\s*(?P<footprint>[0-9]*.[0-9]*)\s*kg CO2eq"
    ),
    re.compile(r"(?P<energy_demand>[0-9]*.[0-9]*)\s*kW[h]*\s*ENERGY STAR"),
    re.compile(
        r"associated with product use is (?P<energy_demand>[0-9]*.[0-9]*)\s*kWh\s*per year"
    ),
    ...
    re.compile(
        r"(?P<date>[A-Z][a-z]+(?:\s+[0-9]?[0-9],)? [0-9]{4})\s*©\s*[0-9]{4}\s*Microsoft\s*Corporatio"
    ),
    ...
    re.compile(
        r"Transport(ation)?\s*\(?\<?(?P<gwp_transport_ratio>[0-9]*\.*[0-9]*)\s*%"
    ),
    ...
    re.compile(r"Product (U|u)se\s*\(?(?P<gwp_use>[0-9]*\.*[0-9]*)\s*kg\s*CO"),
    re.compile(r"Manufacturing\s*\(?\<?(?P<gwp_manuf>[0-9]*\.*[0-9]*)\s*kg\s*CO"),
    re.compile(r"Production\s*\(?\<?(?P<gwp_manuf>[0-9]*\.*[0-9]*)\s*kg\s*CO"),
    re.compile(
        r"Transport(ation)?\s*\(\<?(?P<gwp_transport>[0-9]*\.*[0-9]*)\s*kg\s*CO"
    ),
    re.compile(r"Distribution\s*\(?\<?(?P<gwp_transport>[0-9]*\.*[0-9]*)\s*kg\s*CO"),
    re.compile(r"Disposal\s*\(?\<?(?P<gwp_eol>[0-9]*\.*[0-9]*)\s*kg\s*CO"),
    re.compile(r"End of (L|l)ife\s*\(?\<?(?P<gwp_eol>[0-9]*\.*[0-9]*)\s*kg\s*CO"),
)
...
_DATE_PATTERN = re.compile(
    r"([A-Z][a-z]+(?:\s+[0-9]?[0-9],)? [0-9]{4})\s*©\s*[0-9]{4}\s*Microsoft\s*Corporatio"
)
...
def parse(body: BinaryIO, pdf_filename: str) -> Iterator[data.DeviceCarbonFootprint]:
    ...
    result["name"] = first_page_text.replace("ECOPROFILE", "").strip()
    ...
    if "date" in extracted:
        ...
        for block, page in pdf.search_text(
            body, "Microsoft Corporation. All rights reserved"
        ):
            date_text = page.get_textbox(
                (block.x0 - 30, block.y0 - 10, block.x1, block.y1 * 2.1 - block.y0)
            )
    ...
    for block, page in pdf.search_text(body, "Physical features"):
        weight_text = page.get_textbox(
            (block.x0, block.y0, block.x1 + 10, block.y1 - 30)
        )
    ...
    if "lifetime" in extracted:
        ...
        if numeric_lifetime := _ENGLISH_TO_NUMERIC.get(lifetime):
            ...
            result["lifetime"] = float(lifetime)
    ...
    if "gwp_use_ratio" in extracted:
        ...
        if "gwp_total" in result:
            if "gwp_use" in extracted:
                result["gwp_use_ratio"] = round(
                    float(extracted["gwp_use"]) / result["gwp_total"], 3
                )
    if "gwp_manuf_ratio" in extracted:
        result["gwp_manufacturing_ratio"] = round(
            float(extracted["gwp_manuf_ratio"]) / 100, 3
        )
    else:
        if "gwp_total" in result:
            if "gwp_manuf" in extracted:
                result["gwp_manufacturing_ratio"] = round(
                    float(extracted["gwp_manuf"]) / result["gwp_total"], 3
                )
    if "gwp_eol_ratio" in extracted:
        ...
        if "gwp_total" in result:
            if "gwp_eol" in extracted:
                result["gwp_eol_ratio"] = round(
                    float(extracted["gwp_eol"]) / result["gwp_total"], 3
                )
    if "gwp_transport_ratio" in extracted:
        result["gwp_transport_ratio"] = round(
            float(extracted["gwp_transport_ratio"]) / 100, 3
        )
    else:
        if "gwp_total" in result:
            if "gwp_transport" in extracted:
                result["gwp_transport_ratio"] = round(
                    float(extracted["gwp_transport"]) / result["gwp_total"], 3
                )
