...
_HPE_PATTERNS = (
    ...
    re.compile(
        r"ESTIMATED PRODUCT CARBON FOOTPRINT\:\*?\s*(?P<footprint>[0-9]*)\s*kg\s*CO2\s*e"
    ),
    re.compile(
        r"The mean carbon footprint for this product is\s*(?P<footprint>[0-9]*)\s*kg\s*CO2\s*e"
    ),
    ...
    re.compile(
        r"Use location\s*(?P<use_location>(EU|US|Europe|North America|China|WW|Worldwide))"
    ),
    re.compile(
        r"nergy demand \((Y|y)early TEC\)\s*(?P<energy_demand>[0-9]*\.[0-9]*)\s*kWh"
    ),
    ...
    re.compile(
        r"Assembly location\s*(?P<assembly_location>(EU|US|Europe|North America|China|WW|Worldwide))"
    ),
    ...
    re.compile(
        r"Daughterboard[^0-9]*(?P<gwp_daughterboard>[0-9]*\.?[0-9]*)\s*kg\s*CO2"
    ),
...
def parse(body: BinaryIO, pdf_filename: str) -> Iterator[data.DeviceCarbonFootprint]:
    ...
    if "tolerance" in extracted and "gwp_total" in result:
        result["gwp_error_ratio"] = round(
            (float(extracted["tolerance"]) / result["gwp_total"]), 3
        )
    ...
    if "cpu_quantity" in extracted:
        result["number_cpu"] = float(extracted["cpu_quantity"])  # type: ignore
    ...
    if "gwp_ssd_ratio" in extracted:
        ...
        if "gwp_total" in result:
            if "gwp_ssd" in extracted:
                result["gwp_ssd_ratio"] = round(
                    float(extracted["gwp_ssd"]) / result["gwp_total"], 3
                )
    ...
    if "gwp_mainboard_ratio" in extracted:
        ...
        if "gwp_total" in result:
            if "gwp_mainboard" in extracted:
                result["gwp_electronics_ratio"] += (
                    float(extracted["gwp_mainboard"]) / result["gwp_total"]
                )
    if "gwp_daughterboard_ratio" in extracted:
        result["gwp_electronics_ratio"] += (
            float(extracted["gwp_daughterboard_ratio"]) / 100
        )
    else:
        if "gwp_total" in result:
            if "gwp_daughterboard" in extracted:
                result["gwp_electronics_ratio"] += (
                    float(extracted["gwp_daughterboard"]) / result["gwp_total"]
                )
    if "gwp_enclosure_ratio" in extracted:
        result["gwp_othercomponents_ratio"] += (
            float(extracted["gwp_enclosure_ratio"]) / 100
        )
    else:
        if "gwp_total" in result:
            if "gwp_enclosure" in extracted:
                result["gwp_othercomponents_ratio"] += (
                    float(extracted["gwp_enclosure"]) / result["gwp_total"]
                )
    if "gwp_assembly_ratio" in extracted:
        result["gwp_othercomponents_ratio"] += (
            float(extracted["gwp_assembly_ratio"]) / 100
        )
    else:
        if "gwp_total" in result:
            if "gwp_assembly" in extracted:
                result["gwp_othercomponents_ratio"] += (
                    float(extracted["gwp_assembly"]) / result["gwp_total"]
                )
