...
from .utils import piechart_analyser
...
_HP_DESK_PATTERNS = (
    ...
    re.compile(
        r"Product (c|C)arbon (f|F)ootprint (Report)*\s*(?P<name>.{0,50}?)\s*GHG"
    ),
    re.compile(r"Estimated impact (?P<footprint>[0-9]*)\s*kgCO2 eq."),
    re.compile(r"herein.(?P<footprint>[0-9]*)\s*kg\s*CO2eq."),
    re.compile(
        r"Other\s*organizations\s*might\s*report\s*this\s*value\s*(as)?\s*(?P<footprint_with_error>[0-9]*)\s*\+\/\-\s*(?P<tolerance>[0-9]*)\s*kg\s*of\s*CO2-e"
    ),
    re.compile(
        r"Estimated\s*impact\s*(?P<footprint_with_error>[0-9]*)\s*kgCO2e(q)?\s*\+\/\-\s*(?P<tolerance>[0-9]*)\s*kg\s*(of)?\s*CO2.?e"
    ),
    re.compile(r"Lifetime of product\s*(?P<lifetime>[0-9]*) years"),
    re.compile(
        r"Use location\s*(?P<use_location>(Europe|North America|China|WW|Worldwide))"
    ),
    re.compile(
        r"Use energy demand \(Yearly TEC\)\s*(?P<energy_demand>[0-9]*\.[0-9]*)\s*kWh"
    ),
    re.compile(r"Product weight\s*(?P<weight>[0-9]*\.?\s*[0-9]*)\s*kg"),
    re.compile(r"Screen size\s*(?P<screen_size>[0-9]*\.?\s*[0-9]*)\s*inches"),
    re.compile(
        r"Final manufacturing location\s*(?P<assembly_location>(Europe|North America|China|WW|Worldwide))\s+"
    ),
...
_MANUF_LOCATION_PATTERNS = {
    re.compile(
        r"nufacturing location\s*(?P<assembly_location>(Europe|North America|China|WW|Worldwide))"
    ),
...
def parse(body: BinaryIO, pdf_filename: str) -> Iterator[data.DeviceCarbonFootprint]:
    ...
    if "name" in extracted:
        ...
        logging.error(
            'The file "%s" did not match the HP pattern (no name extracted)',
            pdf_filename,
        )
    ...
    if "footprint_with_error" in extracted and "tolerance" in extracted:
        ...
        result["gwp_error_ratio"] = round(
            (float(extracted["tolerance"]) / result["gwp_total"]), 4
        )
    ...
    if "weight" in extracted:
        ...
        for block, page in pdf.search_text(body, "weight"):
            temp_text = page.get_textbox(
                (block.x0, block.y0 - 2, block.x1 + 150, block.y1 + 2)
            )
            extracted_weight = text.search_all_patterns(_WEIGHT_PATTERNS, temp_text)
            if "weight" in extracted_weight:
                result["weight"] = extracted_weight["weight"]  # type: ignore
                break
    if "screen_size" in extracted:
        ...
        for block, page in pdf.search_text(body, "screen size"):
            ...
            if "screen_size" in extracted_temp:
                result["screen_size"] = extracted_temp["screen_size"]  # type: ignore
                break
    if "assembly_location" in extracted:
        ...
        for block, page in pdf.search_text(body, "manufacturing location"):
            temp_text = page.get_textbox(
                (block.x0, block.y0 - 2, block.x1 + 160, block.y1 + 2)
            )
            extracted_temp = text.search_all_patterns(
                _MANUF_LOCATION_PATTERNS, temp_text
            )
    ...
    if "gwp_manufacturing_ratio" in extracted:
        result["gwp_manufacturing_ratio"] = (
            float(extracted["gwp_manufacturing_ratio"]) / 100
        )
    ...
    if not "gwp_use_ratio" in extracted:
        ...
        for image in pdf.list_images(body):
            md5 = hashlib.md5(image).hexdigest()  # type: ignore
    ...
    if "gwp_use_ratio" in result and "yearly_tec" in result:
        ...
        elec_factor = (
            result["gwp_use_ratio"]
            * result["gwp_total"]  # type: ignore
            / (result["lifetime"] * result["yearly_tec"])  # type: ignore
        )
        if math.isfinite(elec_factor):
            ...
            for expected_factor in [0.686, 0.525]:
                expected_lifetime = elec_factor * result["lifetime"] / expected_factor  # type: ignore
                if (
                    (elec_factor < 0.52 or elec_factor > 0.695)
                    and (abs(expected_lifetime - result["lifetime"]) > 0.6)  # type: ignore
                    and abs(expected_lifetime - round(expected_lifetime)) < 0.1
                ):
                    ...
                    elec_factor = (
                        result["gwp_use_ratio"]
                        * result["gwp_total"]  # type: ignore
                        / (result["lifetime"] * result["yearly_tec"])
                    )
                    break
            ...
            if elec_factor > 0.34 and elec_factor < 0.46:
                ...
                result["gwp_total"] *= correction_factor  # type: ignore
