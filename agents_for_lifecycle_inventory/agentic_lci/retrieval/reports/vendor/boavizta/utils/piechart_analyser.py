...
class PiechartAnalyzer:
    def __init__(self, profileFile=None, debug=0):
        ...
        if not profileFile:
            profileFile = os.path.join(os.path.dirname(__file__), "profiles.json")
    ...
    def auto_prod(self, piedata):
        if not "prod" in piedata:
            ...
            if "use" in piedata and "transp" in piedata:
                ...
                if abs(details - piedata["prod"] > 1.0):
                    self.print(
                        1,
                        "WARNING: production != sum of sub-components (",
                        piedata["prod"],
                        details,
                    )
            elif details > 0:
    ...
    def append_to_boavizta(self, boaitem, piedata):
        ...
        for k, v in toBoa.items():
            if k in piedata:
                if k in main:
                    ...
                    other_sum += piedata[k]
                if v in boaitem:
                    boaitem[v] = round(boaitem[v] + piedata[k] / 100.0, 3)
                else:
                    boaitem[v] = round(piedata[k] / 100.0, 3)

        # sanity checks
        if abs(100.0 - main_sum) > 0.5:
            self.print(
                1,
                "WARNING sum of use+manufacturing+transport+eol != 100 (",
                main_sum,
                ")",
            )
        if "prod" in piedata and abs(piedata["prod"] - other_sum) > 1e-2 * other_sum:
            self.print(
                1,
                "WARNING sum of sub manufacturing components != manufacturing (",
                other_sum,
                "vs",
                piedata["prod"],
                ")",
    ...
    def create_legend_from_ocr(self, input_img, img_no_charts, profile, cimg):
        ...
        rect_kernel = cv2.getStructuringElement(
            cv2.MORPH_RECT, (18, profile["vertical dilation"])
        )
        ...
        for cnt in contours:
            ...
            fulltext = pytesseract.image_to_string(
                cropped, config=custom_oem_psm_config
            ).strip()
            ...
            for text in fulltext.splitlines():
                ...
                if not label_out:
                    if len(text) > 2:
                        self.print(
                            1, "WARNING failed to find matching label for ", text
                        )
                    continue
                ...
                cv2.rectangle(
                    cimg,
                    (max(0, x - actualH), actualY),
                    (x + 2, actualY + actualH),
                    (0, 0, 255),
                    1,
                )
                cropped = input_img[
                    actualY : actualY + actualH, max(0, x - actualH) : x + 2
                ]
                # binarize
                x0 = max(0, x - actualH)
                y0 = actualY
                ...
                thret, th2 = cv2.threshold(
                    thin, 0, 255, cv2.THRESH_OTSU | cv2.THRESH_BINARY_INV
                )
                # get contour
                if (
                    self.debug >= 4
                    and (not thin is None)
                    and len(thin.shape) >= 2
                    and thin.shape[0] > 0
                    and thin.shape[1] > 0
                ):
                ...
                contours2, hierarchy2 = cv2.findContours(
                    th2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
                )
                if len(contours2) > 0:
                    areas = [
                        cv2.contourArea(c) / max(1, cv2.arcLength(c, True))
                        for c in contours2
                    ]
                    ...
                    self.print(
                        1,
                        "WARNING failed to find color picking contour for '",
                        text,
                        "'",
                    )
    ...
    def percent_from_ocr(self, input_img, circle, profile, cimg):
        ...
        if "ocr percent dilate size" in profile:
            ...
            K2 = cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE, (K2_width, max(1, round(s[1] * circle[2])))
            )
        ...
        for cnt in contours:
            ...
            w -= 2 * shrink
            cv2.rectangle(
                cimg, (x0 + x, y0 + y), (x0 + x + w, y0 + y + h), (255, 255, 0), 1
            )
            ...
            if h < 50:
                ...
                cropped = cv2.resize(
                    cropped, (factor * w, factor * h), interpolation=cv2.INTER_AREA
                )
                    ...
                    if ret:
                        try:
                            ...
                            if value > 100:
                                self.print(
                                    1,
                                    "WARNING: read percentage value",
                                    value,
                                    "for",
                                    label_out,
                                    "is greater than 100%! -> skip it.",
                                )
                            else:
                                if label_out in res:
                                    self.print(
                                        1,
                                        'WARNING: found duplicate for "',
                                        label_out,
                                        '" with values ',
                                        res[label_out],
                                        " and ",
                        ...
                        self.print(
                            1,
                            "WARNING: cannot find associated percentage for",
                            label_out,
                        )
    ...
    def analyze(self, input_img, ocrprofile=None):
        ...
        img_id = (
            np.left_shift(img[:, :, 0].astype(np.int32), 16)
            + np.left_shift(img[:, :, 1].astype(np.int32), 8)
        ...
        cv2.rectangle(
            img_no_charts,
            (0, 0),
            (img_no_charts.shape[1], img_no_charts.shape[0]),
            (255, 255, 255),
            5,
        )
        ...
        cv2.rectangle(
            img_no_charts, (rmin0, rmin1), (rmax0, rmax1), (255, 255, 255), -1
        )
        for circle in circles:
            circle = np.uint16(np.around(circle))
            if circle[2] > dmin / 4:
        ...
        if "ocr patterns legend" in mainprofile:
            autolegend = self.create_legend_from_ocr(
                input_img, img_no_charts, mainprofile, cimg
            )
        ...
        for circle in circles[: min(2, len(circles))]:
            ...
            if (
                (circle[0] - circle[2] < 0)
                or (circle[1] - circle[2] < 0)
                or (circle[0] + circle[2] >= img.shape[1])
                or (circle[1] + circle[2] >= img.shape[0])
            ):
            ...
            if autolegend:
                ...
                self.print(
                    2,
                    "autolegend scoring: {}+{} / {}".format(
                        score, count_opt, len(autolegend)
                    ),
                )
                ...
                if score > 0:
                    ...
                    for ck, c in autolegend.items():
                        ...
                        if ind.size > 0:
                            ...
                            if (
                                ...
                                or (
                                    "opt" in mainprofile
                                    and item_name not in mainprofile["opt"]
                                )
                                or float(q) / float(total_px) > 0.0015
                            ):  # filter noise
                                res[item_name] = q
                                trueSum += q
                                count += 1
                    ...
                    if trueSum / total_px > 0.5:
                        ...
                        is_details_pie_charts = (
                            "profile/main" in res and res["profile/main"] == "ocr"
                        )
                        if (not is_details_pie_charts) and (not "EOL" in res):
                            self.print(
                                1, "WARNING: missed end-of-life, add it explicitely..."
                            )
                            res["EOL"] = 0
                            count += 1
                        ...
                        if is_details_pie_charts and "prod" in res:
                            factor = res["prod"] / 100.0
                        for ck, c in res.items():
                            if ck in inserteditems:
                                res[ck] = (
                                    round(10000 * (c + corr) / total_px * factor)
                                    / 100.0
                                )

                        if "profile/main" in res:
                            res["confidence2"] = round(100 * trueSum / total_px) / 100.0
                            res["profile/prod"] = "auto legend"
                        else:
                            res["confidence1"] = round(100 * trueSum / total_px) / 100.0
                            res["confidence2"] = 1
            ...
            if try_ocr:
                self.print(
                    2,
                    "No profile is good enough, we should have trueSum/total_px = ",
                    trueSum / total_px,
                    " > ",
                    0.5,
                )
                if not res and "ocr patterns direct" in mainprofile:
                    res, malus = self.percent_from_ocr(
                        input_img, [circle[0], circle[1], re], mainprofile, cimg
                    )
                    if res:
                        is_details_pie_charts = (
                            "profile/main" in res and res["profile/main"] == "ocr"
                        ...
                        if not is_details_pie_charts:
                            ...
                            if part:
                                ...
                                if not "EOL" in res:
                                    self.print(
                                        1,
                                        "WARNING: missed end-of-life, add it explicitely...",
                                    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        default=0,
        type=int,
        help="show circles and debug info",
    )
