...
_APPLE_PATTERNS = (
    ...
    re.compile(
        r"(?P<gwp_manufacturing_ratio>[0-9]+\.?[0-9]{0,2})\%[^a-zA-Z0-9]*Production"
    ),
    re.compile(r"(?P<gwp_transport_ratio>[0-9]+\.?[0-9]{0,2})\%[^a-zA-Z0-9]*Transport"),
    re.compile(
        r"(?P<gwp_use_ratio>[0-9]+\.?[0-9]{0,2})\%[^a-zA-Z0-9]*(Use|Customer use)"
    ),
    re.compile(
        r"(?P<gwp_eol_ratio>[0-9]+\.?[0-9]{0,2})\%[^a-zA-Z0-9]*(End-of-life processing|Recycling)"
    ),
)
...
def parse(body: BinaryIO, pdf_filename: str) -> Iterator[data.DeviceCarbonFootprint]:
    ...
    result["name"] = result["name"].replace(" storage", "")  # type: ignore
    ...
    if "lifetime" in extracted:
        ...
        if numeric_lifetime := _ENGLISH_TO_NUMERIC.get(lifetime):
            ...
            result["lifetime"] = lifetime  # type: ignore
    ...
    test = re.findall(
        r"(?P<storage>[0-9]+[A-Z]B)\s*(?P<impact>[0-9]+\.?[0-9]*)", pdf_as_text
    )
    result["comment"] = ""
    for i in test:
        result["comment"] += (
            extracted["name"] + " " + i[0] + " (" + i[1] + "kgCO2eq) - "
        )
