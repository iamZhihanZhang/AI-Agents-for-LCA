...
from .utils.image import crop, find_text_in_image, image_to_text
...
_DELL_LCA_PATTERNS = (
    ...
    re.compile(
        r" estimated carbon footprint\: \s*(?P<footprint>[0-9]*) kgCO2e(?: \+\/\- (?P<error>[0-9]*)\s*kgCO2e)?"
    ),
    re.compile(
        r" estimated ?standard deviation (of )?\+\/\- (?P<error>[0-9]*)\s*kgCO2e"
    ),
    ...
    re.compile(
        r" Energy Demand \(Yearly TEC\)\s*(?P<energy_demand>[0-9]*.[0-9]*)\s*kWh"
    ),
...
_MANUF_PERCENT_PATTERN = re.compile(
    r".*nufac[a-z0-9]*[a-z][^0-9\.]([0-9]*\.*[0-9]*)\%.*"
)
_EOL_PERCENT_PATTERN = re.compile(r".*EoL([0-9]*\.*[0-9]*)\%.*")
_TRANSPORT_PERCENT_PATTERN = re.compile(
    r".*port[A-Za-z]*[^0-9\.]?([0-9]*\.*[0-9]*)\%.*"
)
...
def parse(body: BinaryIO, pdf_filename: str) -> Iterator[data.DeviceCarbonFootprint]:
    ...
    if result.get("gwp_total") and "error" in extracted:
        result["gwp_error_ratio"] = round(
            (float(extracted["error"]) / result["gwp_total"]), 3
        )
